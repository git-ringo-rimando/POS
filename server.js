...
app.put('/api/categories/:id', auth, adminOnly, async (req, res) => {
  ...
  try {
    await sql.begin(async sql => {
      ...
      await sql`UPDATE products SET category=${name.trim()}, updated_at=CURRENT_TIMESTAMP WHERE category=${oldName}`;
      await sql`INSERT INTO category_archive (category_name, action, old_name, changed_by_name) VALUES (${name.trim()}, 'renamed', ${oldName}, ${req.user.full_name || req.user.username})`;
    });
    res.json({ success: true });
...
app.get('/api/product-history', auth, adminOnly, async (req, res) => {
  res.json(await sql`SELECT * FROM product_history ORDER BY changed_at DESC LIMIT 500`);
});

// ── Reports ────────────────────────────────────────────────────────────────────
app.get('/api/reports/summary', auth, adminOrManager, async (req, res) => {
